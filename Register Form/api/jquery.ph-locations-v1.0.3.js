...
(function ($, window, document, undefined) {
  ...
  var pluginName = "ph_locations",
    defaults = {
      ...
      api_base_url:
        "https://fk350plrl6.execute-api.us-east-1.amazonaws.com/prod/",
      order: "name asc",
      filter: {},
      default_values: {
  ...
  $.extend(Plugin.prototype, {
    ...
    fetch_list: function (options) {
      if (options.filter != undefined) this.settings.filter = options.filter;
    ...
    map_parameters() {
      ...
      for (var property in this.settings.filter)
        mapped_parameter.filter.where[property] =
